...
fn type_infer_attribute_expr(
    ...
    for _ in 0..MAX_RETRIES {
        ...
        let expr = inline_global_consts(
            ...
            &state.function.parameters.iter().map(|(_, _, param_name, _, _)| param_name).collect(),
        )
        .map_err(|e| MonomorphizationErrorBundle::ResolverError(e))?;
    ...
    if last_crate_id.map_or(true, |id| id != current_func_crate_id) {
        *globals_with_paths = monomorphizer
            ...
            .map(|global_info| {
                ...
                // NOTE: The function `fully_qualified_module_path` fails to add consistently `::`
                // add the end of each path. Therefore we manually add double colons if they are missing.
