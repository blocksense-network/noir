...
impl<'a> SsaPass<'a> {
    ...
    pub(crate) fn new_if<F>(condition: bool, f: F, msg: &'static str) -> Option<Self>
    where
        F: Fn(Ssa) -> Ssa + 'a,
    {
        if condition { Some(SsaPass::new(f, msg)) } else { None }
    }

    pub(crate) fn new_try_if<F>(condition: bool, f: F, msg: &'static str) -> Option<Self>
    where
        F: Fn(Ssa) -> Result<Ssa, RuntimeError> + 'a,
    {
        if condition { Some(Self { msg, run: Box::new(f) }) } else { None }
    }
}
...
pub fn primary_passes(options: &SsaEvaluatorOptions) -> Vec<SsaPass> {
    ...
    vec![
        Some(SsaPass::new(Ssa::remove_unreachable_functions, "Removing Unreachable Functions")),
        SsaPass::new_if(!debug, Ssa::defunctionalize, "Defunctionalization"),
        SsaPass::new_if(!debug, Ssa::inline_simple_functions, "Inlining simple functions"),
        ...
        Some(SsaPass::new(Ssa::remove_paired_rc, "Removing Paired rc_inc & rc_decs")),
        SsaPass::new_if(
            !debug,
        ...
        Some(SsaPass::new(Ssa::mem2reg, "Mem2Reg")),
        SsaPass::new_if(!debug, Ssa::simplify_cfg, "Simplifying"),
        Some(SsaPass::new(Ssa::as_slice_optimization, "`as_slice` optimization")),
        Some(SsaPass::new(Ssa::remove_unreachable_functions, "Removing Unreachable Functions")),
        Some(SsaPass::new_try(
            Ssa::evaluate_static_assert_and_assert_constant,
            "`static_assert` and `assert_constant`",
        )),
        Some(SsaPass::new(Ssa::purity_analysis, "Purity Analysis")),
        SsaPass::new_if(!debug, Ssa::loop_invariant_code_motion, "Loop Invariant Code Motion"),
        SsaPass::new_try_if(
            !debug,
            move |ssa| ssa.unroll_loops_iteratively(options.max_bytecode_increase_percent),
            "Unrolling",
        ),
        Some(SsaPass::new(Ssa::simplify_cfg, "Simplifying")),
        Some(SsaPass::new(Ssa::mem2reg, "Mem2Reg")),
        SsaPass::new_if(!debug, Ssa::flatten_cfg, "Flattening"),
        Some(SsaPass::new(Ssa::remove_bit_shifts, "Removing Bit Shifts")),
        ...
        SsaPass::new_if(!debug, Ssa::remove_if_else, "Remove IfElse"),
        Some(SsaPass::new(Ssa::purity_analysis, "Purity Analysis")),
        SsaPass::new_if(!debug, Ssa::fold_constants, "Constant Folding"),
        SsaPass::new_if(
            !debug,
            Ssa::flatten_basic_conditionals,
            "Simplify conditionals for unconstrained",
        ),
        Some(SsaPass::new(Ssa::remove_enable_side_effects, "EnableSideEffectsIf removal")),
        SsaPass::new_if(!debug, Ssa::fold_constants_using_constraints, "Constraint Folding"),
        Some(SsaPass::new(
            Ssa::make_constrain_not_equal_instructions,
            "Adding constrain not equal",
        )),
        SsaPass::new_if(!debug, Ssa::check_u128_mul_overflow, "Check u128 mul overflow"),
        Some(SsaPass::new(Ssa::dead_instruction_elimination, "Dead Instruction Elimination")),
        SsaPass::new_if(!debug, Ssa::simplify_cfg, "Simplifying"),
        Some(SsaPass::new(Ssa::array_set_optimization, "Array Set Optimizations")),
        ...
        SsaPass::new_if(!debug, Ssa::brillig_entry_point_analysis, "Brillig Entry Point Analysis"),
        ...
        Some(SsaPass::new(Ssa::brillig_array_gets, "Brillig Array Get Optimizations")),
        Some(SsaPass::new(Ssa::dead_instruction_elimination, "Dead Instruction Elimination")),
    ]
    .into_iter()
    .flatten()
