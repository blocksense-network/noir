...
mod tests {
    ...
    use crate::{
        ...
        token::{SecondaryAttribute, SecondaryAttributeKind},
    };
    ...
    fn parse_multiple_fv_attributes_with_function() {
        ...
        let SecondaryAttribute {
            kind:
                SecondaryAttributeKind::FvAttribute(FormalVerificationAttribute::Ensures(req_attribute)),
            location: _,
        } = function_secondary_attrs.last().unwrap()
        else {
    ...
    fn pares_fv_attributes_cooperate() {
        ...
        assert!(matches!(
            function_secondary_attrs[1].kind,
            SecondaryAttributeKind::Deprecated(None)
        ));
    }
}
