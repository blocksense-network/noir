...
impl<'a, B: BlackBoxFunctionSolver<FieldElement>> TracingContext<'a, B> {
    ...
    fn update_record(&mut self, tracer: &mut Tracer, is_tracer_started: &mut bool) {
        if !*is_tracer_started {
            tracer.start(
                &PathBuf::from(self.current_filepath.to_string()),
                Line(self.current_line as i64),
            );
            *is_tracer_started = true;
        } else {
...
pub fn trace_circuit<B: BlackBoxFunctionSolver<FieldElement>>(
    ...
    let mut steps = 0;
    let mut is_tracer_started = false;
    loop {
        ...
        tracing_context.update_record(tracer, &mut is_tracer_started);
    }
